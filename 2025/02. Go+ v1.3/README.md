...
* 丰富的函数。函数太多不会带来理解上的负担（就像这个世界上人名很多一样）。运算符（operator）也是函数。所以 Go+ 支持的运算符非常多（例如它提供了 `->`、`<>` 运算符），builtin 函数的规模也远大于 Go 语言。Go+ 可以说是最在意领域友好（SDF）程度的语言，只要某种运算符在某个领域被广泛接纳，Go+ 可以几乎无负担地为其增加新运算符。
